...
def add_sphere(polygons, cx, cy, cz, r, step ):
    ...
    step+= 1
    for l in range(lat_start, lat_stop):
        for l1 in range(longt_start, longt_stop):
            i = l * step + l1

            add_polygon(polygons, points[i][0], points[i][1], points[i][2], points[(i + 1) % len(points)][0], points[(i + 1) % len(points)][1], points[(i + 1) % len(points)][2], points[(i + step) % len(points)][0], points[(i + step) % len(points)][1], points[(i + step) % len(points)][2])
            add_polygon(polygons, points[(i + 1) % len(points)][0], points[(i + 1) % len(points)][1], points[(i + 1) % len(points)][2], points[(i + step + 1) % len(points)][0], points[(i + step + 1) % len(points)][1], points[(i + step + 1) % len(points)][2], points[(i + step) % len(points)][0], points[(i + step) % len(points)][1], points[(i + step) % len(points)][2])
